...
const Container:React.FC<Props> = (props) => {
    const [state, dispatch] = useReducer<React.Reducer<TypeState, TypeAction>>(reducer, initTd);
    const [toadd, setToadd] = useState<string>("");
    return (
        <div className="container">
            <button onClick={props.click1}>Bấm để đổi màu Header</button>
            <AddList>
                <input placeholder="Nhập việc cần làm" onChange={(e)=>setToadd(e.target.value)}></input>
                <button onClick={() => {dispatch({type: ActionTypes.ADD , id: 0 , value: toadd}); setToadd("")}}>Add</button>
            </AddList>
            {
                state.td.map(
                    (item: any) => {
